...
func TestSearchAction_Success(t *testing.T) {
	...
	mockClient.On("Search", mock.Anything, &pb.SearchAccountRequest{Search: "test-url"}).Return(
		&pb.SearchAccountResponse{Acc: []*pb.Account{{Id: "1", Login: "test", Url: "test-url", Description: "desc"}}}, nil,
	)
	...
	assert.Contains(t, output, "test-url")
	mockClient.AssertExpectations(t)
}

func TestSearchAction_NoURL(t *testing.T) {
	// Подготовка тестовых данных
	testInput := ""
...
func TestSearchAction_SearchError(t *testing.T) {
	...
	// Проверяем результаты
	assert.Contains(t, output, "Ошибка при получении списка аккаунтов.")
	mockClient.AssertExpectations(t)
}
