...
func TestRemoveAction_Success(t *testing.T) {
	...
	mockClient.On("Delete", mock.Anything, &pb.DeleteCardRequest{ID: "test-id"}).Return(
	...
	assert.Contains(t, output, "Карта успешно удалена")
	mockClient.AssertExpectations(t)
}

func TestRemoveAction_NoID(t *testing.T) {
	// Подготовка тестовых данных с пустым ID
	testInput := strings.Join([]string{
		"", // Пустой id
		"", // Новая строка в конце
	}, "\n")
...
func TestRemoveAction_DeleteError(t *testing.T) {
	// Подготовка тестовых данных
	testInput := strings.Join([]string{
		"test-id", // id
		"",        // Новая строка в конце
	...
	// Проверяем результаты
	assert.Contains(t, output, "Ошибка удаления карты")
	mockClient.AssertExpectations(t)
}
