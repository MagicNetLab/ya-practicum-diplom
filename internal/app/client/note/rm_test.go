...
func TestRemoveAction_Success(t *testing.T) {
	...
	// Настраиваем мок для метода Remove
	mockClient.On("Remove", mock.Anything, &pb.RemoveNoteRequest{ID: "test-id"}).Return(
		&pb.RemoveNoteResponse{}, nil,
	)
	...
	assert.Contains(t, output, "Заметка удалена")
...
func TestRemoveAction_ScanError(t *testing.T) {
...
func TestRemoveAction_RemoveError(t *testing.T) {
	...
	// Создаем мок клиента с ошибкой удаления заметки
	mockClient := new(mocks.NoteClient)
	mockClient.On("Remove", mock.Anything, &pb.RemoveNoteRequest{ID: "test-id"}).Return(
	...
	// Проверяем результаты
	assert.Contains(t, output, "Ошибка при удалении заметки")
	mockClient.AssertExpectations(t)
}
